...
class CppAdapter(LanguageAdapter):
    ...
    def _get_parser(self):
        ...
        if self._parser is None:
            try:
                # Use tree-sitter-cpp package if available
                import tree_sitter
                from tree_sitter_cpp import language
                
                CPP_LANGUAGE = tree_sitter.Language(language())
                
                self._parser = tree_sitter.Parser()
                self._parser.language = CPP_LANGUAGE
                print(f"Debug: C++ parser initialized successfully", file=sys.stderr)
            except ImportError as e:
                print(f"Warning: tree-sitter-cpp not available: {e}", file=sys.stderr)
                self._parser = None
        ...
        for path in paths:
            if os.path.isfile(path):
                ...
                for root, dirs, files in os.walk(path):
                    ...
                    dirs[:] = [d for d in dirs if not d.startswith('.') and d not in ['__pycache__', 'build', 'cmake-build-debug']]
...
default_cpp_adapter = CppAdapter()
