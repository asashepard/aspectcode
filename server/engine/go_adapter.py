...
class GoAdapter(LanguageAdapter):
    """Tree-sitter adapter for Go language."""
    
    def __init__(self, parser_path: Optional[str] = None):
        """Initialize Go adapter with tree-sitter parser."""
        self._parser = None
    ...
    def _get_parser(self):
        ...
        if self._parser is None:
            try:
                # Use tree-sitter-go package if available
                import tree_sitter
                from tree_sitter_go import language
                
                GO_LANGUAGE = tree_sitter.Language(language())
                
                self._parser = tree_sitter.Parser()
                self._parser.language = GO_LANGUAGE
                print(f"Debug: Go parser initialized successfully", file=sys.stderr)
            except ImportError as e:
                print(f"Warning: tree-sitter-go not available: {e}", file=sys.stderr)
                self._parser = None
            except Exception as e:
                print(f"Warning: Could not initialize Go parser: {e}", file=sys.stderr)
                self._parser = None
    ...
    def list_files(self, paths: List[str]) -> List[str]:
        """List all Go files in the given paths."""
        go_files = []
        ...
        for path in paths:
            if os.path.isfile(path):
                ...
                for root, dirs, files in os.walk(path):
                    ...
                    dirs[:] = [d for d in dirs if not d.startswith('.') and d not in ['__pycache__', 'vendor']]
        ...
        def find_enclosing_function(node):
            if hasattr(node, 'start_byte') and hasattr(node, 'end_byte'):
                if node.start_byte <= byte_offset <= node.end_byte:
                    if hasattr(node, 'type') and node.type in ['function_declaration', 'method_declaration']:
            ...
            if hasattr(node, 'type') and node.type == 'binary_expression':
                ...
                for child in node.children:
                    if hasattr(child, 'type'):
                        if child.type in ['identifier', 'int_literal', 'string_literal', 'selector_expression', 'call_expression', 'parenthesized_expression']:
