...
class JavaAdapter(LanguageAdapter):
    """Tree-sitter adapter for Java language."""
    
    def __init__(self, parser_path: Optional[str] = None):
        """Initialize Java adapter with tree-sitter parser."""
        self._parser = None
    ...
    def _get_parser(self):
        ...
        if self._parser is None:
            try:
                # Use tree-sitter-java package if available
                import tree_sitter
                from tree_sitter_java import language
                
                JAVA_LANGUAGE = tree_sitter.Language(language())
                
                self._parser = tree_sitter.Parser()
                self._parser.language = JAVA_LANGUAGE
                print(f"Debug: Java parser initialized successfully", file=sys.stderr)
            except ImportError as e:
                print(f"Warning: tree-sitter-java not available: {e}", file=sys.stderr)
                self._parser = None
            except Exception as e:
                print(f"Warning: Could not initialize Java parser: {e}", file=sys.stderr)
                self._parser = None
    ...
    def list_files(self, paths: List[str]) -> List[str]:
        """List all Java files in the given paths."""
        java_files = []
        ...
        for path in paths:
            if os.path.isfile(path):
                ...
                for root, dirs, files in os.walk(path):
                    ...
                    dirs[:] = [d for d in dirs if not d.startswith('.') and d not in ['__pycache__', 'target', 'build']]
            ...
            if hasattr(node, 'type') and node.type == 'binary_expression':
                ...
                for child in node.children:
                    if hasattr(child, 'type'):
                        if child.type in ['identifier', 'decimal_integer_literal', 'string_literal', 'field_access', 'method_invocation', 'parenthesized_expression']:
    ...
    def iter_symbol_defs(self, tree):
        ...
        scope_types = {
            'program', 'class_declaration', 'method_declaration', 
            'constructor_declaration', 'block', 'static_initializer',
            'lambda_expression', 'for_statement', 'enhanced_for_statement',
        ...
        def visit_node(node, parent_scope='module', parent_class=None, current_scope_id=0):
            ...
            if node.type == 'class_declaration':
                ...
                if name_node:
                    ...
                    # Visit class body for methods/fields
                    for child in node.children:
            ...
            # Field declarations: private int x;
                ...
                for child in node.children:
                    if hasattr(child, 'type'):
                        if child.type == 'scoped_identifier':
                            ...
                            import_path = self._get_scoped_identifier(child)
                        elif child.type == 'identifier':
                            import_path = [self._node_text_to_str(child.text)]
                
                if import_path:
                    # Use the last part as the name (e.g., "List" from "java.util.List")
                    name = import_path[-1]
                    full_module = '.'.join(import_path[:-1]) if len(import_path) > 1 else ''
    ...
    def _get_scoped_identifier(self, node):
        """Extract parts from a scoped_identifier node."""
        parts = []
                ...
                if parent and hasattr(parent, 'type'):
                    ...
                    if parent.type in ['class_declaration', 'method_declaration', 
                        ...
                        for i, child in enumerate(parent.children):
                            if child == node:
                                # First identifier in these is typically the name
                                if parent.type == 'variable_declarator' and i == 0:
                                    is_declaration = True
                                elif parent.type in ['formal_parameter', 'catch_formal_parameter']:
                                    is_declaration = True
                                elif parent.type in ['class_declaration', 'method_declaration', 'constructor_declaration']:
                                    is_declaration = True
                                break
                    
                    # Skip import identifiers
                    if parent.type in ['import_declaration', 'scoped_identifier']:
                        is_declaration = True
...
default_java_adapter = JavaAdapter()
