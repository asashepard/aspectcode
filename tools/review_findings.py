...
def resolve_server_root(explicit: Optional[str] = None) -> Optional[Path]:
    """Resolve the server root for locating rule implementation files.

    The OSS VS Code extension can run without the Python server.
    These maintainer tools can optionally point at a server checkout to map
    rule IDs to implementation files.
...
def find_rule_implementation(rule_id: str, server_root: Optional[Path]) -> List[str]:
    """
    Find the implementation file(s) for a given rule_id.
    
    Searches <server_root>/rules/ for files containing the rule_id string.
...
def build_rule_to_file_map(server_root: Optional[Path]) -> Dict[str, List[str]]:
    """
...
def main():
    ...
    parser.add_argument(
        "--server-root",
        help=(
            "Path to the Aspect Code server checkout (folder containing rules/). "
    ...
    # Handle --find-rule
    if args.find_rule:
        matches = find_rule_implementation(args.find_rule, server_root)
        if matches:
            print(f"Rule '{args.find_rule}' is implemented in:")
            for m in matches:
                if server_root:
                    rel_path = os.path.relpath(m, server_root)
                    print(f"  {server_root.name}/{rel_path}")
                else:
                    print(f"  {m}")
        else:
            print(f"No implementation found for rule '{args.find_rule}'")
            if not server_root:
                print("Tip: pass --server-root /path/to/server or set ASPECTCODE_SERVER_ROOT")
        return
    
    # Handle --list-rules
    if args.list_rules:
        rule_map = build_rule_to_file_map(server_root)
        if rule_map:
            ...
            for rule_id in sorted(rule_map.keys()):
                ...
                for f in files:
                    if server_root:
                        rf = os.path.relpath(f, server_root)
                        print(f"    -> {server_root.name}/{rf}")
                    else:
                        print(f"    -> {f}")
        else:
            if not server_root:
                print("No server checkout found. Pass --server-root or set ASPECTCODE_SERVER_ROOT.")
            else:
                print(f"No rules found in {server_root / 'rules'}")
        return
