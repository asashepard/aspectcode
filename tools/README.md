# Aspect Code Internal Tools

**INTERNAL USE ONLY** - These tools are for maintainers, not end users.

## Manual Alpha Review Harness
...
Before releasing alpha, we need to verify that our rules don't produce bogus 
findings on real codebases. This harness:
...
5. **For each finding, ask**:
   ...
   - Is this a false positive? → Either:
     - Add guards to the rule in `server/rules/`
     - Add path filtering in `server/engine/file_filter.py`
     - Disable the rule in `server/engine/profiles.py` (set `enabled=False`)
...
- `server/scripts/run_alpha_findings_review.py` - Single-repo analysis (lower level)
- `server/engine/profiles.py` - Alpha profile rule list (where to disable rules)
- `server/engine/file_filter.py` - File filtering logic (where to add path exclusions)
...
5. **Fix noisy rules**:
   ...
   # Then either:
   # a) Add guards in server/rules/<rule>.py
   # b) Add path filtering in server/engine/file_filter.py
   # c) Disable in server/engine/profiles.py (set enabled=False)
   ```
...
**Total findings:** 96
**Implementation:** server/rules/ident_shadowing.py
